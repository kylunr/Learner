...
class Calendar extends React.Component {
    ...
    state = {
        ...
        eventsIndex: 0,
        colorCounter: 0,
        colorArray: ['blue', 'red', 'orange', 'yellow', 'green']
    };
                ...
                <div className="subjects_section">
                    ...
                    <div className="form">
                        <form onSubmit={e => e.preventDefault()}>
                            ...
                            <button type="submit" className="submit_button" onClick={this.createSubject}>Submit</button>
                        </form>
                    </div>
        ...
        docRef.onSnapshot(function(doc) {
            ...
            if (doc.exists) {
                ...
                if (!(document.subjects === "")) {
                    console.log(document.subjects);
                    this.setState({
                        userSubjects: document.subjects.split('$$') 
                    });
                } 
                ...
                if (!(document.calendarEvents === "")) {
                    ...
                    eventsArray.forEach(element => {
                        ...
                        newCalendarEvents.push(
                            ...
                            {title: element.title, start: element.start.toDate(), end: element.end.toDate(), id: element.id}
                        );
    ...
    createSubject = () => {
        ...
        let newSubjects = "";
        this.state.userSubjects.forEach(element => newSubjects += (element + "$$"));
    ...
    deleteSubject = (event) => {
        ...
        if (numberOfSubjects === 1) {
            ...
            var index = this.state.userSubjects.indexOf(event.target.id);
    ...
    handleDrop = (event) => {
        ...
        tempArray.push(
            {title: event.event.title, start: event.event.start, end: event.event.end, id: parseInt(this.state.eventsIndex)}
        );
    ...
    handleChange = (event) => {
        ...
        // this.state.calendarEvents[event.event.id] = {title: event.event.title, start: event.event.start, end: event.event.end, id: event.event.id};

        let tempArray = this.state.calendarEvents.slice();
        tempArray[event.event.id] = {title: event.event.title, start: event.event.start, end: event.event.end, id: event.event.id};
